  ...
  return new Promise((resolve, reject) => {
    ...
    if (fs.existsSync(`${destination}/${fileName}.page`)) {
        ...
        terminal.prompt(
          ...
          (answer) => {
            if (answer === "Y" || answer === "y") {
              deleteWebPage(fileName, destination).then((stat) => {
                if (stat === "success")
                  updatePageCount(fileName, "remove", args);
              });

              createWebPage(fileName, args, destination).then((stat) => {
                if (stat === "success")
      ...
      if (args.react_cli || args.vue_cli) {
        ...
        fs.createWriteStream(
          `${destination}/${fileName}.page/index.${args.php ? "php" : "html"}`
        );
      }
      ...
      if (!args.no_css) {
        ...
        if (args.react_cli || args.vue_cli) {
          let pathToPrimaryStylesheet = `${destination}/${fileName}.page/src/styles/${fileName}.css`,
            pathToContentStylesheet = `${destination}/${fileName}.page/src/styles/${fileName}.content.css`;
          ...
          if (args.responsive) {
            ...
            [320, 375, 425, 768, 1024, 1440, 2160].forEach((width) => {
              fs.appendFile(
                `${destination}/${fileName}.page/src/styles/res/${fileName}.res.${width.toString()}px.css`,
          ...
          let pathToPrimaryStylesheet = `${destination}/${fileName}.page/${fileName}.styles/--${fileName}.css`,
            pathToContentStylesheet = `${destination}/${fileName}.page/${fileName}.styles/${fileName}.content.css`;
